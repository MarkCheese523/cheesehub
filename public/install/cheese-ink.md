...
```bash
mkdir -p <SKILLS_DIR>
curl -fsSL -o /tmp/cheese-ink.zip http://localhost:7100/skills/cheese-ink.zip
unzip -o /tmp/cheese-ink.zip -d <SKILLS_DIR>/
```

解压后应存在 `<SKILLS_DIR>/cheese-ink/SKILL.md`。若 `localhost:7100` 不可达，说明用户的 CheeseHub 站点没在运行，请向用户索要 zip 文件路径后改用本地文件解压。
