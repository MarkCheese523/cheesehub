  ...
  return (
    <footer className="relative overflow-hidden border-t border-foreground/10">
      <div className="mx-auto max-w-6xl px-4 pt-16 sm:px-6">
          ...
          {COLUMNS.map((col) => (
            <div key={col.title}>
              ...
              <ul className="mt-4 space-y-2.5">
                {col.links.map((link) => (
                  <li key={link}>
                    <span className="group inline-flex cursor-pointer items-center gap-1 font-mono text-xs text-muted-foreground transition-colors hover:text-lime-600 dark:hover:text-lime-400">
                      {link}
                      {col.title === 'Ecosystem' && (
        ...
        <div className="mt-14 flex items-center justify-between border-t border-foreground/5 py-6 font-mono text-[11px] text-muted-foreground/70">
      ...
      <div
        aria-hidden
        className="pointer-events-none select-none whitespace-nowrap text-center font-black leading-[0.75] tracking-tighter text-foreground/[0.04]"
